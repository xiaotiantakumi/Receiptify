...
## 開発時の注意点
  ...
  - **Zodによる型管理（重要）**: このプロジェクトではzodライブラリを使用した型定義とバリデーションを標準とします。新しいAPI関数やデータ構造を追加する際は、必ず以下を実装してください：
    - `api/src/schemas/validation.ts` でのzodスキーマ定義
    - `api/src/lib/validation-helpers.ts` のヘルパー関数の使用
    - OWASP基準に準拠したセキュリティチェック
    - 統一されたエラーハンドリング
    - フロントエンドでも同様にzodスキーマを共有して型安全性を確保すること