...
export const processReceipt: StorageBlobHandler = async (blob: unknown, context: InvocationContext): Promise<void> => {
  ...
  const userId = containerName.split('/').pop()?.replace('user-', ''); // コンテナ名からユーザーIDを抽出

  if (!userId) {
    context.error('Unable to extract user ID from container name');
    return;
  }
  ...
  try {
    ...
    await saveReceiptResult(userId, receiptId, {
      receiptImageUrl: `${containerName}/${blobName}`,
      status: 'completed',
